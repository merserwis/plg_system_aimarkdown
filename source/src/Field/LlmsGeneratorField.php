 ...
 */
class LlmsGeneratorField extends FormField
{
    protected $type = 'LlmsGenerator';
    ...
    protected function getInput(): string
    {
        ...
        <div class="card bg-light border p-3 my-2 shadow-sm">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h6 class="mb-1 fw-bold text-dark">Status pliku /llms.txt</h6>
                    <small class="text-muted">Lokalizacja: <code><?php echo htmlspecialchars($filePath, ENT_QUOTES, 'UTF-8'); ?></code></small>
                </div>
                <div>
                    <?php if ($fileExists): ?>
                        <span class="badge bg-success">Plik istnieje na dysku</span>
                    <?php else: ?>
                        <span class="badge bg-secondary">Brak pliku</span>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="row g-2 text-center mb-3">
                <div class="col-md-6">
                    <div class="p-2 bg-white rounded border">
                        <div class="small text-muted text-uppercase">Ostatnia modyfikacja</div>
                        <div class="fw-bold" id="llms-date"><?php echo htmlspecialchars($fileDate, ENT_QUOTES, 'UTF-8'); ?></div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="p-2 bg-white rounded border">
                        <div class="small text-muted text-uppercase">Rozmiar pliku</div>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <button type="button" 
                    ...
                    <span class="icon-refresh" aria-hidden="true"></span> Wygeneruj / Zaktualizuj /llms.txt teraz
                </button>
                <?php if ($fileExists): ?>
                    <a href="<?php echo htmlspecialchars($fileUrl, ENT_QUOTES, 'UTF-8'); ?>" 
                       ...
                       class="btn btn-outline-secondary">
                        <span class="icon-eye" aria-hidden="true"></span> Zobacz /llms.txt
                    </a>
                <?php endif; ?>
            </div>
        ...
        function generateLlmsTxtNow(btn) {
            ...
            btn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Generowanie...';
            ...
            .then(data => {
                ...
                if (data && data.success) {
                    ...
                    msgBox.textContent = 'Błąd: ' + (data.message || 'Nieznany błąd');
                    msgBox.style.display = 'block';
                }
            })
            .catch(err => {
                ...
                btn.innerHTML = originalText;
                msgBox.className = 'alert alert-danger mt-3 py-2 small';
                msgBox.textContent = 'Błąd połączenia podczas generowania pliku.';
                msgBox.style.display = 'block';
            });
        }
        </script>
